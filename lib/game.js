...
class Game {
  constructor(ctx, gameCanvas, backgroundCtx, foregroundCtx) {
    ...
    this.trees = 0;
    this.maxTrees = 3;
    this.backgroundMusic = new Audio('./assets/sounds/paragon-x-chaos.mp3');
  ...
  generateObstacle(tree = false) {
    let obstacle = null;
    if (tree || Math.random() < 0.5) {
      let sprite = 'light';
      if (Math.random() > 0.5) {
        sprite = 'dark';
      }
      obstacle = new Tree({ startPos: [820, 140], speed: 5, sprite: sprite });
    } else {
      obstacle = new Bird({ startPos: [820, 48], speed: 6.3 });
    }
    return obstacle;
  }
